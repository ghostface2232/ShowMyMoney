...
- Page structure from top to bottom: persistent header → horizontal summary card strip → monthly asset table. The header ("OO의 자산 관리" + action buttons) lives in the `(app)` group layout (`src/app/(app)/layout.tsx`) so it stays mounted across tab navigations; only the right-side actions crossfade by route (`src/components/header-actions.tsx`).
