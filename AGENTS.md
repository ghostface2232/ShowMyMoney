...
- The app has two top-level routes switched via a fixed iOS-style bottom tab bar (`src/components/tab-bar.tsx`): `/` (assets) and `/expenses` (expenses). Pages add bottom padding (`pb-24`) so content clears the bar. The expenses page loads one month at a time via the `?ym=` search param and follows the same structure: header → month nav → summary card strip (total / per member / shared, tap to filter scope) → category breakdown → day-grouped transaction list with an inline composer. Expense delta badges invert the asset colors (spending increase = destructive). Page-level month titles use `formatYearMonthLong` ("2026년 6월"); table columns keep the compact `formatYearMonth` ("26_6월").
