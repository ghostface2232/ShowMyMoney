...
- The app has two top-level routes switched via a floating pill-shaped bottom tab bar (`src/components/tab-bar.tsx`): `/` (assets) and `/expenses` (expenses). The bar is rendered in the root layout OUTSIDE MotionShell — a transformed/will-change ancestor becomes the containing block for `position: fixed` and would pin the bar to page content instead of the viewport. It hides itself on auth routes and pages add bottom padding (`pb-24`) so content clears it. Both routes have `loading.tsx` skeletons so tab navigation paints instantly; same-route searchParams changes (e.g. expense month nav) run inside a transition and keep the current content instead of flashing the skeleton.
