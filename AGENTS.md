...
- Write all code comments and documentation (README.md, AGENTS.md) in English. Keep them concise — essential information only, no filler.

## UI Conventions

- App-shell scrolling: the document never scrolls. The root layout (`src/app/layout.tsx`) locks `body` to the viewport (`h-full overflow-hidden`) and wraps all routes in a fixed-height scroll container (`#app-scroll`). This stops Safari's toolbars from collapsing/expanding on scroll, so the fixed bottom tab bar never gets pushed around; standalone (home screen) mode behaves identically. Any code that reads or sets scroll position must use `getScrollContainer()` from `src/lib/scroll-memory.ts`, never `window.scrollY` / `window.scrollTo`.
- Page structure from top to bottom: persistent header → horizontal summary card strip → monthly asset table. The header ("OO의 자산 관리" + action buttons) lives in the `(app)` group layout (`src/app/(app)/layout.tsx`) so it stays mounted across tab navigations. Both right-side action sets stay mounted in the same grid cell and swap opacity (`src/components/header-actions.tsx`) — the header height must never change between tabs, or the whole page shifts mid-transition. Tab links navigate with `scroll={false}` and each tab's scroll offset is saved/restored via `src/lib/scroll-memory.ts`.
- The app has two top-level routes switched via a floating pill-shaped bottom tab bar (`src/components/tab-bar.tsx`): `/` (assets) and `/expenses` (expenses). The bar is rendered in the root layout OUTSIDE MotionShell — a transformed/will-change ancestor becomes the containing block for `position: fixed` and would pin the bar to page content instead of the viewport. It hides itself on auth routes and pages add bottom padding (`pb-28`) so content clears it. Both routes have `loading.tsx` skeletons so tab navigation paints instantly; same-route searchParams changes (e.g. expense month nav) run inside a transition and keep the current content instead of flashing the skeleton. Tab bar links use `prefetch={true}` and `experimental.staleTimes` (next.config.ts) keeps both pages in the client router cache, so switching tabs reuses the cached payload instead of refetching; server actions invalidate that cache via `revalidatePath` after writes. MotionShell (mounted in the `(app)` layout, below the header) slides content horizontally in the direction of tab travel and falls back to fade + y elsewhere. Entry animations (card stagger, bar growth) play only before the first client navigation of the session — gate them with `hasClientNavigated()` from `src/lib/app-navigation.ts`.
- The expenses page loads one month at a time via the `?ym=` search param and follows the same structure: header → month nav → summary card strip (total / per member / shared, tap to filter scope) → category breakdown → day-grouped transaction list with an inline composer. Expense delta badges invert the asset colors (spending increase = destructive). Page-level month titles use `formatYearMonthLong` ("2026년 6월"); table columns keep the compact `formatYearMonth` ("26_6월"). Expense create/update/delete are optimistic, mirroring the asset table's local-mirror + runInBackground pattern.
...
- The service worker may cache an older version, so a double refresh (hard reload) is recommended when testing after deploy.
- Serwist is auto-disabled in local development (`disable: process.env.NODE_ENV === "development"`). To verify service worker behavior, run a production build with `npm run build && npm run start`.
- PWA icons are temporary PNGs (SMM initials) under `public/icons/`; replace them at the same paths/sizes once final branding is decided.
- The service worker does not use `@serwist/next`'s `defaultCache`. The default cache stores HTML/RSC/API GET responses keyed by URL, but this app's responses are distinguished only by the account cookie, risking cross-account data leakage on shared devices. Instead, cache only static assets (`_next/static`, images, fonts, etc.), and handle HTML documents, RSC payloads, `/api/*`, and all other same-origin requests as `NetworkOnly`. On SW upgrade, the `activate` handler purges any runtime cache buckets outside the safe list. When adding a new runtime cache, always review whether its URL includes the account or is distinguished only by the auth cookie.